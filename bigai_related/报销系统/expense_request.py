...
class InvoiceProcessor:
            ...
            if text.strip():
                ...
                if invoice_number and invoice_number != "识别失败":
                    if invoice_number in seen_invoice_numbers:
                        logger.warning(
                            f"发现重复发票号码: {invoice_number} (文件: {pdf_file.name})"
                        )
                        if "备注" not in info:
                            info["备注"] = ""
    ...
    def create_excel_report(self, invoice_data: List[Dict[str, str]], output_path: str):
        ...
        with pd.ExcelWriter(output_path, engine="openpyxl") as writer:
            df.to_excel(
                writer, sheet_name="报销明细", index=False, startrow=1
            )  # 从第2行开始写入数据
            ...
            dv_invoice_type = DataValidation(
                type="list", formula1=f'"{invoice_type_options}"'
            )
            ...
            dv_payment_type = DataValidation(
                type="list", formula1=f'"{payment_type_options}"'
            )
            ...
            dv_subject_detail = DataValidation(
                type="list", formula1=f'"{subject_detail_options}"'
            )
